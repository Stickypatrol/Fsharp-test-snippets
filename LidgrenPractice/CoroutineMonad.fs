...
let rec repeatFor_ r i =
    co { if i < 0 then
             do! r
             return! repeatFor_ r (i - 1)
         else
             return () }
...
//if coroutine doesnt return Done() then return ()
let tryOnce c =
  fun w so si ->
    match c w so si with
    | Done(so, si, a) -> Done(so, si, ())
    | _ -> Done(so, si, ())
//if coroutine doesnt return Done() then return x
let tryOnceWith c x =
  fun w so si ->
    match c w so si with
    | Done(so, si, a) as result -> result
